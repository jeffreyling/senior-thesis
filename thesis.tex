...
The first methods were rule-based. % discuss SYSTRAN and linguistics
...
Recently, researchers found that deep learning works effectively for many NLP tasks. For the first time, neural networks were able learning structure and features from language almost completely from scratch \cite{Collobert2011} % \cite{Bengio2003} language model
The success of neural methods has been adopted by Google Translate to build even better systems \cite{GoogleTranslate2016}. These methods have attracted attention from the press % NYTimes???
...
\citet{mikolov2013word2vec} show that training deep networks can also work on language. They were able to map words in the English language to a vector of real numbers using purely unsupervised methods on a Google News corpus. % diagram of vectors, perhaps an example of the analogy (king man woman queen)

Deep learning methods have since come to bear in NLP. \cite{goldberg2015primer} provides a nice summary. % more here? talk about LSTMs?

Deeper and deeper networks become more and more feasible as modern computing power increases. Today, GPUs are leveraged to rapidly train networks at 10 or 20 times the speed of standard CPUs.  % cite some numbers here
...
Second, removing assumptions about the input to a system allows us to build more general models -- this has already proven to be useful in machine translation, where one system can be trained on multiple languages \cite{Firat2016}. If we used a system of parsers  %% not sure this argument works. seq2seq can't be trained on low resource languages.
...
Smmry's algorithm is a simple extractive summarization method. It counts word occurrences, splits discussions by sentence, and ranks the sentences based on the sum of their word scores (perhaps tf-idf?). This algorithm is reminiscent of %%% the first guy?
...
%% coarse to fine: nallapati is not the same!,  \cite{mei2016}
...
% talk about: Q-learning, other algorithms (SARSA?)
...
\chapter{Models}
% figures needed!!!
% also I should start from the beginning probably
