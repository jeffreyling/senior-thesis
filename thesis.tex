...
With this goal in mind, we attempt to interpretably extend the existing sequence-to-sequence model with attention, and we analyze the performance on the task of document summarization. In the next chapter, we provide the background for how we will do this.
...
\begin{equation}
\boldh^s = \text{max-over-time}(\tanh(\boldu))
\end{equation}
where $\text{max-over-time}$ takes the maximum along the $j$-indexing dimension.
...
Since samples from $\balpha_t$ at time $t$ of the RNN decoder can also affect future rewards, we use a discount factor of $\gamma = 0.5$, so that the reward is $\hat{C}_t = \sum_{s = t}^n \gamma^{n-s}r_s$ at time $t$, where $r_t = \log p(y_t | y_1, \ldots, y_{t-1})$ is the single step reward.
